...
public:
	// Enhanced Input绑定函数
	void OnPrimaryActionStarted(const FInputActionValue& Value);
	void OnPrimaryActionCompleted(const FInputActionValue& Value);
...
protected:
	...
	// 左键按下时是否落在战斗 UI 上，用于避免 UI 点击同时穿透到地图
	bool bPrimaryActionStartedOverUI = false;

	bool bIsInspectingUnit = false;
