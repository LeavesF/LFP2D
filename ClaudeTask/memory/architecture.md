...
## 15. 遗物商店系统 (completed)
最小版本商店已落地，当前定位为**遗物商店**，不负责卖单位；卖单位的版本后续单独命名为“雇佣市场”。

### 数据层
- `LFPRelicTypes.h`：定义遗物效果、遗物定义、商店条目、商店定义
- `ULFPRelicDataAsset`：`RelicID -> FLFPRelicDefinition`
- `ULFPShopDataAsset`：`ShopID -> FLFPShopDefinition`
- `ULFPGameInstance`：持有 `OwnedRelicIDs`、`RelicDataAsset`、`ShopDataAsset`

### 配置模式
- 世界地图节点只挂 `ShopID`，不直接存商品内容
- `FLFPWorldNodeRow` / `ALFPWorldMapNode` / 世界地图 CSV / 编辑器笔刷参数已接入 `ShopID`
- 商店内容通过 `ShopID + DataAsset` 查找

### UI 与入口
- `ULFPShopWidget`：商店面板，显示金币、遗物列表、详情、购买按钮
- `ALFPWorldMapPlayerController`：
  - `WNT_Shop` 节点可直接打开商店
  - Town 中点击 `TBT_Shop` 建筑可打开商店
  - 从 Town 打开的商店关闭后会返回 TownWidget

### 购买逻辑
- `ULFPGameInstance::TryPurchaseRelic()`：校验遗物存在、未拥有、金币足够，然后扣钱并写入 `OwnedRelicIDs`
- 当前保留“购买 + 持有 + 展示”闭环，不要求战斗内立即生效
